...
async function installRoutingRule() {
  ...
  const block = [
    ...
    "## 绘图任务委派",
    "",
    "创建或修改可编辑图表时，必须委派给 `drawing-master`。包括流程图、架构图、时序图、ER 图、思维导图、UML、状态图、泳道图、组织架构图、甘特图、时间线、树形图、网络拓扑图、数据流图、概念图、鱼骨图、SWOT、金字塔图、漏斗图、韦恩图、矩阵图、信息图，以及明确提及 Excalidraw、Mermaid、draw.io 或 diagram 的任务。",
    "",
    "始终先委派，由 `drawing-master` 确认格式。用户未指定格式或相应扩展名时，必须询问并允许多选；主、子代理都不得默认格式。",
    "",
    "艺术插画、统计绘图、CAD 和普通图片生成不得委派。自动委派未发生时，提示用户显式使用 `@drawing-master`。",
    end,
  ].join("\n");
